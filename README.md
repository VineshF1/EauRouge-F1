...
Fine-tuned `unsloth/Qwen3-8B-unsloth-bnb-4bit` (299k dl) on `VineshF1/F1-Dataset` 10639 QA — fixes `7 vs 5 vs 4` hallucination.

## Model
🤗 **HF:** `Vinesh/EauRouge-F1-Qwen3-8B-v2` *(push after test — link will be live)* → https://huggingface.co/Vinesh/EauRouge-F1-Qwen3-8B-v2

Dataset: https://github.com/VineshF1/F1-Dataset `10639` pairs `5745 race_result · 2039 qual · 1748 driver_bio · 364 champion · 221 career_total · 177 circuit`

Base: `unsloth/Qwen3-8B-unsloth-bnb-4bit` 4.8% hallucination (Vectara) — T4 16GB

## Training
* Colab T4, Unsloth, QLoRA r32 alpha32, `max_seq 1024` packed, `batch 4x2` eff 8, `1 epoch` 101 steps ~35 min, `lr 2e-4` cosine `warmup 0.03`, `eval 10%` 1064
